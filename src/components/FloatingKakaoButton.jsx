...
function FloatingKakaoButton() {
  ...
  return (
    <div
      className={`fixed bottom-4 md:bottom-6 right-4 md:right-6 z-50 transition-all duration-500 ${
        isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
      }`}
    >
      <button
        onClick={handleClick}
        className="group relative bg-white hover:bg-gray-50 active:scale-95 rounded-full p-1.5 md:p-2 shadow-2xl hover:shadow-2xl transition-all duration-300 md:hover:scale-110 flex items-center gap-1 md:gap-2"
      ...
      >
        ...
        {/* 펄스 효과 */}
        <span className="absolute -top-0.5 md:-top-1 -right-0.5 md:-right-1 flex h-2.5 w-2.5 md:h-3 md:w-3">
